...
def team(request, eid=None):
    ...
    if request.method == "POST":
        ...
        if not re.match(email_pattern, empEmail) or empEmail.rfind(".") <= empEmail.index('@')+2:
            messages.error(request, "Please Enter Valid Email Address")
        elif(not empTerms):
            messages.error(request, "Please accept terms and conditions below the form")
        else:
            if eid:
                if Team.objects.filter(empEmail=empEmail).exclude(id=eid).exists():
                    messages.error(request, "Employee with this email Already registered")
                else:
...
def service(request, pid):
    ...
    technician_positions = ['laptop_technician', 'computer_technician', 'chip_level_technician']
